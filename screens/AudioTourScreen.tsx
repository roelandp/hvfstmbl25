...
export default function AudioTourScreen() {
  ...
  const loadAudioStops = async () => {
    try {
      ...
      const csvModule = await import('../assets/stops_restructured_cleaned.csv');
      let csvData = '';

      // Try to get CSV data - it might be imported differently
      if (typeof csvModule.default === 'string') {
        csvData = csvModule.default;
      } else {
        // Fallback: fetch the file directly
        try {
          const response = await fetch('./assets/stops_restructured_cleaned.csv');
          csvData = await response.text();
        } catch (fetchError) {
          console.warn('Could not fetch CSV file, using fallback data');
          // Use the existing hardcoded data as fallback
          csvData = `id,title,script_text,lat,lon
